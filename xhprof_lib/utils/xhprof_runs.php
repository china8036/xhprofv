...
class XHProfRuns_Qqes implements iXHProfRuns{
  ...
  public function get_run($run_id, $type, &$run_desc){
          $run_desc = "XHProf Run (Namespace=$type)";
          $xhprof = \RedBeanPHP\R::load('xhprof', $run_id);
          if(!$xhprof){
              throw new Exception('not found this id');
          }
          $xdetail = \RedBeanPHP\R::load('xdetail', $run_id);
          return unserialize($xdetail->content);
